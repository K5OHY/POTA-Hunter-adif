...
def clean_and_parse_log_data(log_data):
    ...
    i = 0
    while i < len(lines):
        # Look for a line that starts with a date and time format (YYYY-MM-DD HH:MM)
        if len(lines[i].strip().split()) == 2 and "-" in lines[i] and ":" in lines[i]:
            try:
                ...
                # Extract the callsign from the next line
                call = lines[i + 1].strip()
                
                # Skip the next line (operator's callsign)
                # Extract the details from the fourth line
                details = lines[i + 3].strip().split()
                
                # Ensure there are enough details to parse
                if len(details) < 5:
                    st.warning(f"Skipping entry due to incomplete details: {lines[i:i+4]}")
                    i += 4
                    continue
                
                station_callsign = details[0].strip()  # Your callsign (e.g., K5OHY)
                ...
                # If there's any error due to unexpected log structure, skip this entry
